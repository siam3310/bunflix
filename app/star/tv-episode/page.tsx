...
export default function Save() {
  ...
  return (
    <>
      ...
      {tvEpisode?.length == 0 && (
        <h1 className=" p-4 font-bold text-xl">Seems like you haven&apos;t Added any TV Shows to your Collection, yet.</h1>
      )}
      <div className=" px-2 grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {tvEpisode?.map((e) => (
          <div
            key={e.id}
            className={`relative inline-block text-white rounded-lg overflow-hidden xl:h-[300px] m-2 transition-all duration-200 `}
          >
            <img
            ...
            <div className="absolute h-full flex movie-center justify-between flex-col top-0 left-0 w-full transition-all bg-black/80 backdrop-blur-md opacity-0 hover:opacity-100 p-3  xl:pl-3 group">
              <div>
                <p
                  className={` whitespace-normal font-semibold mb-2 leading-[16px] transition-all duration-[300ms]  group-hover:leading-[28px] text-2xl `}
                >
                  {e.name}
                </p>
                <p className="text-[13px] md:text-md opacity-10  duration-500 group-hover:opacity-70 ">
                  Episode no. : {e.episode_number}
                </p><p className="text-[13px] md:text-md opacity-10  duration-500 group-hover:opacity-70 ">
            ...
            text-[15px] pt-1 opacity-10"
              ...
              <div className=" flex flex-col gap-2 px-2">
                <Link  href={`/video/tv/${e.show_id}?season=${e.season_number || e.season}&episode=${e.episode_number||e.episode_number}&provider=vidsrc`}>
