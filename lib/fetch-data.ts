...
export async function aniwatchHomeApi() {
  const data = await fetch("https://aniwatch-rho.vercel.app/anime/home");
  if (!data.ok) {
...
export async function fetchAniwatchId(id: string) {
  const data = await fetch(
    `https://aniwatch-rho.vercel.app/anime/info?id=${id}`
  );
  if (!data.ok) {
...
export async function fetchAniwatchEpisode(seasonId: string) {
  const data = await fetch(
    `https://aniwatch-rho.vercel.app/anime/episodes/${seasonId}`
  );
  if (!data.ok) {
...
export async function fetchAniwatchEpisodeServer(
  ...
  const data = await fetch(
    `https://aniwatch-rho.vercel.app/anime/servers?episodeId=${episodeId}?ep=${episode}`
  );
  if (!data.ok) {
...
export async function fetchAniwatchEpisodeSrc(
  ...
  const data = await fetch(
    `https://aniwatch-rho.vercel.app/anime/episode-srcs?id=${episodeId}?ep=${episode}&server=vidstreaming`
  );
  if (!data.ok) {
...
export async function fetchAniwatchEpisodeSrcDub(
  ...
  const data = await fetch(
    `https://aniwatch-rho.vercel.app/anime/episode-srcs?id=${episodeId}?ep=${episode}&server=vidstreaming&category=dub`
  );
  if (!data.ok) {
    throw new Error("Episode Source Fetch Failed");
  }
...
export async function fetchAniwatchSearch(
  ...
  const data = await fetch(
    `https://aniwatch-rho.vercel.app/anime/search?q=${searchTerm}&page=${page}`
  );
  if (!data.ok) {
